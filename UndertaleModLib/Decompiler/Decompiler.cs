...
namespace UndertaleModLib.Decompiler
{
    ...
    public static class Decompiler
    {
        ...
        public static string Decompile(UndertaleCode code, GlobalDecompileContext globalContext, Action<string> msgDelegate = null)
        {
            ...
            foreach (UndertaleCode duplicate in code.ChildEntries)
            {
                if (!context.Statements.ContainsKey(duplicate.Offset / 4)) // agrego esto porque salía un error con weas duplicadas
                {
        ...
        public static void BuildSubFunctionCache(UndertaleData data)
        {
            ...
            Task mainTask = Task.Run(() =>
            {
                ...
                Parallel.ForEach(data.Code, globalScript =>
                {
                    ...
                    if (scriptCode.ParentEntry == null)
                    {
                        processingCodeList[scriptCode.Name.Content] = null;
                        try
                        {
                            ...
                            childContext.DisableAnonymousFunctionNameResolution = true;
                            Dictionary<uint, Block> blocks2 = PrepareDecompileFlow(scriptCode, new List<uint>() { 0 });
                            if (scriptCode.Name.Content == "utils")
                            {
                                Debug.WriteLine("utils here");
                            }
                            DecompileFromBlock(childContext, blocks2, blocks2[0]);
                            List<Statement> statements = HLDecompile(childContext, blocks2, blocks2[0], blocks2[scriptCode.Length / 4]);
                            foreach (Statement stmt2 in statements)
                            {
                                string finalOut = stmt2.ToString();
                                //Debug.WriteLine(finalOut);
                                if (scriptCode.Name.Content == "utils")
                                {
                                    if (stmt2 is AssignmentStatement ass)
                                    {
                                        Debug.WriteLine("name: " + ass.Destination.Var.Name.Content);
                                    }
                                }
                                if (stmt2 is AssignmentStatement assign &&
                                    assign.Value is FunctionDefinition funcDef)
                                {
                                    ...
                                    {
                                        if (scriptCode.Name.Content == "utils")
                                            Debug.WriteLine(assign.Destination.Var.Name.Content + " as " + funcDef.Function.Name.Content);
                                        data.KnownSubFunctions.Add(assign.Destination.Var.Name.Content, funcDef.Function);
                                    }
                                }
                            }
                    ...
                    else if (scriptCode.Name.Content.Contains("gml_Script_"))
                    {
                        string scriptName = scriptCode.Name.Content;
                        string funcName = scriptName.Replace("gml_Script_", "");
                        if (!data.KnownSubFunctions.ContainsKey(funcName))
                        {
                            data.KnownSubFunctions.Add(funcName, funcDict[scriptName]);
                            if (scriptCode.ParentEntry.Name.Content == "utils")
                                Debug.WriteLine("[..] " + funcName + " as " + scriptName);
                        }
                    }
                    /*catch (Exception e)
                    {
                        ...
                        if (funcDict.ContainsKey(globalScript.Name.Content))
                        {
                            UndertaleFunction dummyFunc = funcDict[globalScript.Name.Content];
                            string fName = globalScript.Name.Content.Replace("gml_Script_", "");
                            dummyFunc.Name = new UndertaleString(globalScript.Name.Content);
                            Debug.WriteLine("[..] " + fName + " as " + dummyFunc.Name.Content);
                            if (!data.KnownSubFunctions.ContainsKey(fName))
                            {
                                data.KnownSubFunctions.Add(fName, dummyFunc);
                            }
                        }
                    }*/
